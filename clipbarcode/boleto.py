...
class Arrecadacao(Boleto):
    ...
    def from_linha_digitavel(self, linha_digitavel):
        def validar_linha_digitavel():
            if (
                self.campo1[2] == "8" or self.campo1[2] == "9"
            ):  # TODO: Ao invés desses "ifs" poderíamos fazer uma subclasse de Arrecadação
                ...
                dv_geral = mod11(
                    f"{self.campo1[0:0 + 3]}{self.campo1[4:4 + 8]}{self.campo2}{self.campo3}{self.campo4}",
                    x10=True,
                )
            else:
...
class Cobranca(Boleto):
    ...
    def from_linha_digitavel(self, linha_digitavel):
        ...
        def validar_linha_digitavel():
            ...
            dv = mod11(
                f"{self.id_banco}{self.cod_moeda}{self.fator_venc}{int(round(self.valor * 100)):010}{self.campo_livre_1}{self.campo_livre_2}{self.campo_livre_3}"
            )
            ...
            return (
                ...
                and dv2 == self.dv2
                and dv3 == self.dv3
                and dv == self.dv_geral
            )

        def preencher_cod_barras():
            self.campo_livre_cod_barras = (
                f"{self.campo_livre_1}{self.campo_livre_2}{self.campo_livre_3}"
            )
        ...
        ):  # TODO: Realizar verificações necessárias, como verificar se só possuem números e coisas similares
            self.linha_digitavel = linha_digitavel
    ...
    def from_cod_barras(self, cod_barras):
        ...
        def validar_cod_barras():
            dv = mod11(
                f"{self.id_banco}{self.cod_moeda}{self.fator_venc}{int(round(self.valor * 100)):010}{self.campo_livre_cod_barras}"
            )
